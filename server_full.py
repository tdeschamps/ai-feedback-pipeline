...
# Only import pipeline if settings available (prevents test failures)
try:
...
class BatchTranscriptRequest(BaseModel):
    ...
    transcripts: list[dict[str, str]] = Field(
        ..., description="List of transcripts with id and content"
    )
    metadata: dict[str, Any] | None = Field(default=None, description="Additional metadata")
...
# Initialize FastAPI app conditionally  
...
# Only define routes if FastAPI is properly available
if FASTAPI_AVAILABLE and hasattr(app, 'on_event'):
    ...
    try:
        ...
        result = await pipeline.process_transcript(request.content, request.transcript_id)
...
# Additional routes would go here if FASTAPI_AVAILABLE
# For now, comment them out to avoid import-time errors
...
async def circleback_webhook(
    ...
    try:
        logger.info(
            f"Webhook: Received {payload.event_type} for transcript {payload.transcript_id}"
        )
...
async def process_webhook_transcript(
    pipeline: FeedbackPipeline, content: str, transcript_id: str, metadata: dict[str, Any]
) -> None:
    ...
    try:
        ...
        logger.info(
            f"Background: Completed processing {transcript_id} - "
...
def run_server(host: str = "0.0.0.0", port: int = 8000, workers: int = 1) -> None:  # noqa: S104
    ...
    uvicorn.run("server:app", host=host, port=port, workers=workers, reload=False, log_level="info")
